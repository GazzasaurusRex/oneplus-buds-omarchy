...
| Zhaoyi-ya/OppoPodsManager | Native current/catalogue/select/custom EQ commands and variable-band parsing | Broad OPO device support | GPL-3.0 |
| 1812z/OppoPods | Independent native EQ command corroboration | OPPO-family OPO devices | No standalone licence file found |
...
For native EQ, OppoPodsManager commit `f272e9e` and 1812z/OppoPods commit
`0d9e8a6` independently corroborated `0x010f`, `0x0122`, `0x0406`, and `0x0418`
before hardware writes. Their evidence established a candidate variable-band
format; device-authored replies on each exact reference product then determined
actual support. No source code or packet captured from another model was copied
or blindly sent.
